...
| [`deobfuscate-javascript`](.agents/skills/deobfuscate-javascript/) | Reverse-engineers the bundled JS in `ref/webview/assets` into readable code with meaningful names | `./src/` |
...
Turns the minified, bundled JS extracted into `./ref` back into readable code. It's a multi-stage pipeline (deobfuscate → smart-rename → polish → optional typed `.tsx` rewrite). The agent itself does the semantic renaming; bundled `bun` scripts handle the mechanical work. Restored files land in `./src/` with meaningful names, semantic-domain subfolders, provenance headers, and a shared `src/IMPORT_MAP.json`. Only finalized files land in `./src/`: the skill stages mechanical batch/script output in a hidden working area (`src/.deobfuscate-javascript/`) first, and promotes a file only after it's organized — readable, well-named, clearly structured (and fully typed in deep mode).
...
The individual `bun src/application/*.ts` tools (detect, extract, smart-rename, polish, quality-gate, …) are documented in the skill's [`SKILL.md`](.agents/skills/deobfuscate-javascript/SKILL.md). In normal use you don't call them by hand — the agent orchestrates them.
...
├─ docs/                          # project conventions and architecture docs
├─ scripts/                       # shared pipeline helpers and checks
├─ AGENTS.md                      # symlink to CLAUDE.md (agent entry point)
├─ CLAUDE.md                      # agent guidelines, doc map, and evolution rules
├─ ref/                           # extracted Codex app source (generated by Skill 1)
└─ src/                           # readable, deobfuscated output (generated by Skill 2)
```

`ref/` and `src/` are generated artifacts — regenerate them with the skills rather than editing by hand.

## Agent guidelines

- Read [`CLAUDE.md`](CLAUDE.md) first, then the relevant section of [`docs/`](docs/README.md) before writing code.
- When a boundary is unclear, ask before making assumptions.
- After completing a task, update the relevant doc if behavior, architecture, or conventions changed.

## Checks

```bash
npm run check          # typecheck + test + docs structure
npm run docs:check     # verify docs/ subdirectory READMEs
```

A pre-commit hook also runs `docs:check` and the deobfuscation skill tests before each commit.
