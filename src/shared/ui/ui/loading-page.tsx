...
import { HomepageLogoIcon } from "../../icons/homepage-logo-icon.tsx";
import { svgToDataUrl } from "../../utils/data-url-from-svg.ts";
const codexLogoMask = svgToDataUrl(`
...
function LoadingLogo({ className }: LoadingLogoProps): JSX.Element {
  ...
  return (
    <div aria-hidden="true" className={wrapperClass}>
      ...
      <div
        ...
        style={{
          WebkitMaskImage: `url(${codexLogoMask})`,
          maskImage: `url(${codexLogoMask})`,
...
export function LoadingPage({
  ...
  const wrapperClass = clsx("flex items-center justify-center", bgClass);
  const dragBar =
    overlay || fillParent ? null : (
      <div className="draggable absolute inset-x-0 top-0 electron:h-toolbar extension:h-toolbar-sm" />
    );
  const logoElement = showLogo ? <LoadingLogo className="size-14" /> : null;
