...
// Mermaid diagram arrow geometry utilities — label transforms, arrow offsets,
// and line-position functions with arrow-head compensation.
...
/**
 * Computes an SVG translate string for a label bounding box.
 * When `isCentered` is true, the label is centered at the origin.
 * Otherwise it is offset by the box's x/y position.
 */
export function computeLabelTransform(
  bbox: {
    width: number;
    height: number;
    x?: number;
