...
// Pull request status icons and check state indicators.

import clsx from "clsx";
import { PullRequestOpenIcon } from "../../shared/icons/pull-request-open-icon.tsx";
...
export type PullRequestCheckState =
  ...
  | "ready"
  | "successful";
export type PullRequestTone = "status" | "currentColor" | "neutral";
export interface PullRequestStatusIconProps {
  status: PullRequestStatus;
  className?: string;
  tone?: PullRequestTone;
}
export interface PullRequestCheckIconProps {
  state: PullRequestCheckState;
  className?: string;
  tone?: PullRequestTone;
}
export interface PullRequestButtonLabelProps {
  number?: number | null;
}
function PullRequestDraftIcon(
...
function PullRequestMergedIcon(
  props: React.SVGProps<SVGSVGElement>,
): JSX.Element {
  return (
    <svg
...
function PullRequestFailingIcon(
  props: React.SVGProps<SVGSVGElement>,
): JSX.Element {
  return (
    <svg
...
function PullRequestInProgressIcon(
  props: React.SVGProps<SVGSVGElement>,
): JSX.Element {
  return (
    <svg
...
const checkStateIconMap: Record<
...
const checkStateColorMap: Record<PullRequestCheckState, string> = {
  draft: "text-token-description-foreground",
  failing:
...
const checkStateDotColorMap: Record<string, string> = {
...
const NEUTRAL_COLOR = "text-token-description-foreground";
function getStatusIconConfig(
  status: PullRequestStatus,
  tone: PullRequestTone,
): {
...
function getCheckStateIconConfig(
  state: PullRequestCheckState,
  tone: PullRequestTone,
): {
  Icon: React.FC<React.SVGProps<SVGSVGElement>>;
  ...
  return {
    Icon: checkStateIconMap[state],
    iconClassName:
      tone === "currentColor"
        ? clsx("text-current", checkStateDotColorMap[state])
        : tone === "neutral"
          ? clsx(NEUTRAL_COLOR, checkStateDotColorMap[state])
          : checkStateColorMap[state],
  };
}
export function PullRequestStatusIcon(
  props: PullRequestStatusIconProps,
): JSX.Element {
  const { status, className, tone = "status" } = props;
  const { Icon, iconClassName } = getStatusIconConfig(status, tone);
  return <Icon className={clsx(iconClassName, className)} />;
}
export function PullRequestCheckIcon(
  props: PullRequestCheckIconProps,
): JSX.Element {
  const { state, className, tone = "status" } = props;
  const { Icon, iconClassName } = getCheckStateIconConfig(state, tone);
  return <Icon className={clsx(iconClassName, className)} />;
}
