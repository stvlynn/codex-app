...
// KeyboardShortcutsSearchInput chunk restored from the Codex webview bundle.
import { useIntl, defineMessages } from "react-intl";
import clsx from "clsx";
import { APP_NAME } from "../../../app/app-identity.tsx";
import {
  hasDescription,
  commandMenuGroups,
...
const commandMessages = defineMessages({
  ...
  "codex.command.personalitySettings": {
    ...
    defaultMessage: "Personality settings",
  ...
  "codex.command.keyboardShortcuts": {
    ...
    description: "Command menu item to open keyboard shortcuts settings",
  },
  "codex.command.showKeyboardShortcuts": {
    id: "codex.command.showKeyboardShortcuts",
    defaultMessage: "Show keyboard shortcuts",
    description: "Command menu item to show active keyboard shortcuts",
  },
...
// Native menu title messages
const menuTitleMessages = defineMessages({
...
function getCommandTitleIntl(command: CommandConfig, intl: IntlShape): string {
  const messageId =
    "titleIntlId" in command
      ? command.titleIntlId
      : command.electron?.menuTitleIntlId;
  if (!messageId) return "";
  const msg =
    commandMessages[messageId as keyof typeof commandMessages] ||
    menuTitleMessages[messageId as keyof typeof menuTitleMessages];
  return msg ? intl.formatMessage(msg) : "";
}

export function getCommandTitleAndDescription(
  command: CommandConfig,
  intl: IntlShape,
  overrides?: { title?: string; description?: string },
): { title: string; description: string } {
  const title = overrides?.title ?? getCommandTitleIntl(command, intl);
  const description =
    overrides?.description ??
    (hasDescription(command) && command.descriptionIntlId
      ? intl.formatMessage(
          descriptionMessages[
            command.descriptionIntlId as keyof typeof descriptionMessages
          ],
          { appName: APP_NAME },
        )
      : "");
...
function isCommandAvailable(
  ...
  switch (commandId) {
    case "hotkeyWindow":
      return !!options.isHotkeyWindowEnabled;
    case "switchToMode1":
    case "switchToMode2":
      return !!options.modeSwitchAvailable;
    case "openProcessManager":
      return !!options.isProcessManagerEnabled;
    case "realtimeVoice":
      return !!options.isRealtimeConversationEnabled;
    case "globalDictationHold":
    case "globalDictationToggle":
      return !!(
        options.isGlobalDictationEnabled && options.isVoiceInputEnabled
      );
    default:
      return true;
  }
...
export function isCommandVisible(
  commandId: string,
  options: CommandVisibilityOptions,
): boolean {
  if (!isCommandAvailable(commandId, options)) return false;

  switch (commandId) {
    case "copyConversationPath":
      return options.platform === "macOS";
    case "hardReloadBrowserPage":
    case "openBrowserTab":
    case "reloadBrowserPage":
    case "toggleBrowserPanel":
      return !!options.browserSidebarEnabled;
    case "toggleTraceRecording":
      return false;
    default:
      return true;
  }
...
function getCommandGroupIndex(command: CommandConfig): number {
  if (command.kind !== "webview" || !command.commandMenuGroupKey) {
    return commandMenuGroups.length;
  }
  return commandMenuGroups.indexOf(command.commandMenuGroupKey);
}

export interface KeyboardShortcutsSearchInputProps {
  autoFocus?: boolean;
  ariaLabel?: string;
...
export function KeyboardShortcutsSearchInput({
  ...
  value,
  onValueChange,
}: KeyboardShortcutsSearchInputProps): JSX.Element {
  const intl = useIntl();

  const inputMode = isSearchingByKeystrokes ? "keystrokes" : "text";
  const captureAttribute = isSearchingByKeystrokes || undefined;
  ...
  return (
    <div className="relative">
      <input
        key={inputMode}
        data-codex-shortcut-capture={captureAttribute}
        autoFocus={autoFocus}
