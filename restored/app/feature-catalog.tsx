// Restored from ref/webview/assets/feature-catalog-BEkS6jMH.js
// FeatureCatalog chunk restored from the Codex webview bundle.

import React from "react";
import clsx from "clsx";
export interface PopcornFindBarProps {
  open: boolean;
  query: string;
  summary: string;
  focusToken: unknown;
...
export function PopcornFindBar({
  open,
  query,
  summary,
  focusToken,
  ...
  onNavigateNext,
  onClose,
}: PopcornFindBarProps): JSX.Element | null {
...
export function getFeatureCatalog(artifact?: string): FeatureCatalogItem[] {
  return artifact
    ? featureCatalogItems.filter((item) => item.artifact === artifact)
    : [...featureCatalogItems];
}
export function getFeatureCatalogRows(): [string, string, string, string][] {
  return featureCatalogItems.map((item) => [
    capitalize(item.artifact),
    item.area,
    item.feature,
    item.summary,
  ]);
