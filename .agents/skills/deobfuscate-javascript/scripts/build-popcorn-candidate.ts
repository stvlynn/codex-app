...
import { convertJsxRuntime } from "./.claude/skills/deobfuscate-javascript/scripts/jsx-runtime.ts";
import { splitBundle, type SplitPlan } from "./.claude/skills/deobfuscate-javascript/scripts/split-bundle.ts";
...
const RESTORED_ROOT = "./restored";
const MANIFEST_PATH = "./restored/.deobfuscate-javascript/_full/manifest.json";
const IMPORT_MAP_PATH = "./restored/IMPORT_MAP.json";
...
function computeNewSource(target: string, semanticDir: string): string | null {
  ...
  if (producer?.restored) {
    const rel = path.posix.relative(semanticDir, producer.restored).replace(/\.[cm]?[jt]sx?$/i, "");
...
function customSplitPlan(source: string, publicExport: string): SplitPlan {
  ...
  const reExports: SplitPlan["barrel"]["reExports"] = [];
