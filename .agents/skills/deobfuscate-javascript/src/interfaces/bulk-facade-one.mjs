...
const target = process.argv[4] || "src";
const manifestPath = path.join(target, ".deobfuscate-javascript", "_full", "manifest.json");
...
// Update IMPORT_MAP
const importMapPath = path.join(target, "IMPORT_MAP.json");
let map = {};
try { map = JSON.parse(fs.readFileSync(importMapPath, "utf8")); } catch {}
if (!map.chunks) map.chunks = {};
const exportMap = {};
for (const e of exports) exportMap[e.exported] = e.exported;
map.chunks[basename] = { path: semanticPath, status: "done", exports: exportMap };
fs.writeFileSync(importMapPath, JSON.stringify(map, null, 2) + "\n");
