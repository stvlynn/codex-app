...
function recommendationFor(reports: TechniqueReport[]): string {
  ...
  if (
    ...
    steps.push("src/infrastructure/simplify.ts");
  }
  if (has("control-flow-flattening")) {
    steps.push("src/infrastructure/control-flow-report.ts (then manual unflatten — see SKILL.md)");
  }
  if (steps.length === 0) {
    return "Detected techniques are low-confidence. Inspect the file manually before running transforms.";
  }
  return `Run, in order: ${steps.join(" → ")}. Or run src/infrastructure/deobfuscate.ts to do all at once.`;
}
...
const USAGE = "Usage: bun src/infrastructure/detect.ts <input.js|-> [--out report.json]";
