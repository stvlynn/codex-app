...
import {
  ...
  type ChunkRule,
} from "../domain/chunk-registry.ts";

export { CHUNK_NAME_REGISTRY, extractChunkBasename, type ChunkRule };
...
export const ALIAS_REGISTRY: Record<string, AliasRule> = {
  ...
  ccount: { package: "ccount", style: "named" },
  v4: { package: "uuid", style: "named" },
  queryOptions: { package: "@tanstack/react-query", style: "named" },
};
...
export function resolveNpmImports(
  ...
  for (const path of importPaths) {
    ...
    for (const spec of node.specifiers) {
      ...
      if (res.localName !== info.localName && programPath) {
        const programScope = (programPath as unknown as NodePath<t.Node>).scope;
...
const USAGE =
  "Usage: bun src/infrastructure/resolve-npm-imports.ts <input.js|-> [--out output.js] " +
