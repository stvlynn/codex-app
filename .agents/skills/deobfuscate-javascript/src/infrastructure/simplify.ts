...
function inlineConstants(ast: t.File, stats: SimplifyStats): boolean {
  ...
  traverse(ast, {
    VariableDeclarator(path) {
      ...
      const bindingPath = binding.path as unknown as NodePath;
      candidates.set(key, {
        value: init,
        bindingPath,
  ...
  traverse(ast, {
    Identifier(path0: NodePath<t.Identifier>) {
      const path = path0 as unknown as NodePath<t.Node>;
      if ((path as NodePath<t.Identifier>).isBindingIdentifier({ name: path0.node.name })) return;
      const parentPath = path.parentPath as NodePath<t.Node> | null;
      const parentNode = parentPath?.node;
      if (
        parentPath?.isMemberExpression({ property: path0.node }) &&
        parentNode &&
      ...
      if (
        parentPath?.isObjectProperty({ key: path0.node }) &&
        parentNode &&
      ...
      const binding = path.scope.getBinding(path0.node.name);
      if (!binding) return;
      const identifier = binding.identifier as t.Identifier;
      const key = `${path0.node.name}@${identifier.start ?? 0}`;
...
const USAGE =
  "Usage: bun src/infrastructure/simplify.ts <input.js|-> [--out output.js] [--max-passes 10] [--no-inline]";
