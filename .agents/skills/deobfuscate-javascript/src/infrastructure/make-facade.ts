...
export function makeFacade(
  ...
  if (mode === "reexport") {
    ...
    const header =
      ...
      `//\n` +
      `// ${specifier} re-export boundary (generated by src/infrastructure/make-facade.ts\n` +
  ...
  if (mode === "passthrough") {
    ...
    const header =
      ...
      `//\n` +
      `// RUNTIME PASSTHROUGH (generated by src/infrastructure/make-facade.ts --passthrough).\n` +
  ...
  const header =
    `// Restored from ${provenance}\n` +
    `//\n` +
    `// ${FACADE_MARKER} (generated by src/infrastructure/make-facade.ts). Large vendored/\n` +
...
const USAGE =
  "Usage: bun src/infrastructure/make-facade.ts <chunk.js> [--name-map map.json] " +
