...
function extractRotateCount(arg: t.Node | undefined): number | null {
...
export function transformStringArrays(source: string): StringArrayResult {
  ...
  (programPath as unknown as NodePath<t.Node>).scope.crawl();

  let arraysRemoved = 0;
  for (const entry of collected.values()) {
    const decl = entry.declarator as NodePath<t.VariableDeclarator>;
    if (decl.removed) continue;
    const idNode = decl.node.id;
    if (!t.isIdentifier(idNode)) continue;
    const freshBinding = (
      decl as unknown as NodePath<t.Node>
    ).scope.getBinding(idNode.name);
...
const USAGE = "Usage: bun src/infrastructure/string-array.ts <input.js|-> [--out output.js]";
...
async function main(): Promise<void> {
  ...
  if (stats.arraysCollected === 0) {
    ...
    if (stats.decoderIndirection) {
      console.error(
        "string-array: WARNING — collected arrays but replaced 0 references. Likely a decoder function indirection (e.g., `_0xabc('0x1')`); run src/infrastructure/simplify.ts first or rewrite the decoder manually.",
      );
    }
  }
}
