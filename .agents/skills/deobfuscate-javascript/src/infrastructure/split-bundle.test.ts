...
import { planSplit } from "../application/plan-split.ts";
import { splitBundle, type SplitPlan } from "./split-bundle.ts";
import { analyzeSource, DEFAULT_OPTIONS } from "../domain/quality-gate.ts";
...
describe("split-bundle", () => {
  ...
  test("creates a deterministic split plan for command-keybinding-style exports", () => {
    ...
    const plan = planSplit(source, {
      ...
      outDir: "src/command-keybindings-CvjN6DDf",
      extension: ".ts",
    });

    expect(plan.outDir).toBe("src/command-keybindings-CvjN6DDf");
