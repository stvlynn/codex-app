...
const USAGE =
  "Usage: bun src/infrastructure/polish.ts <input.js|-> [--out output.js] [--report report.json] " +
...
async function main(): Promise<void> {
  ...
  for (const step of result.steps) {
    ...
    } else if (step.step === "dead-shim-elim") {
      const s = step.stats;
      console.error(
        `polish[dead-shim-elim]: ${s.varsRemoved} vars, ` +
          `${s.importsRemoved} import specs, ${s.importDeclarationsRemoved} decls`,
      );
    } else if (step.step === "react-shim-elim") {
      const s = step.stats;
      console.error(
        `polish[react-shim-elim]: ${s.reactNamespacesCollapsed} ns, ` +
          `${s.directMemberShimsCollapsed} members, ${s.importsRemoved} import specs, ` +
          `${s.importDeclarationsRemoved} decls`,
      );
    }
  }
}
