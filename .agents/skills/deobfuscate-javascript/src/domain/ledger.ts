...
const USAGE = `Usage: bun src/domain/ledger.ts <subcommand> [args]
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(paths.manifestPath)) {
    ...
    console.error("run src/domain/build-import-graph.ts first");
    process.exit(1);
  }
  ...
  switch (sub) {
    ...
    case "claim": {
      ...
      if (!basename || !stage || !STAGES.includes(stage)) {
        console.error(`Usage: bun src/domain/ledger.ts claim <basename> <stage>`);
        process.exit(64);
      }
    ...
    case "release": {
      ...
      if (!basename || !stage || !STAGES.includes(stage)) {
        console.error(
          `Usage: bun src/domain/ledger.ts release <basename> <stage>`,
        );
        process.exit(64);
      }
      try {
    ...
    case "mark-done": {
      ...
      if (!basename || !stage || !STAGES.includes(stage)) {
        console.error(
          `Usage: bun src/domain/ledger.ts mark-done <basename> <stage>`,
        );
        process.exit(64);
      }
      const file = manifest.files[basename];
    ...
    case "mark-faced": {
      ...
      if (!basename) {
        console.error(
          `Usage: bun src/domain/ledger.ts mark-faced <basename> [--force]`,
        );
        process.exit(64);
      }
      const file = manifest.files[basename];
    ...
    case "set-organization": {
      ...
      if (!basename) {
        console.error(
          `Usage: bun src/domain/ledger.ts set-organization <basename> --domain <d> --semantic-path <p>`,
        );
        process.exit(64);
      }
    ...
    case "reset": {
      ...
      if (!basename) {
        console.error(
          `Usage: bun src/domain/ledger.ts reset <basename> [--stage rename]`,
        );
        process.exit(64);
      }
