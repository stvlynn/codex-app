...
export function isKnownTerminalBoundaryChunk(
  ...
  const publicPath = (hint.path ?? "").toLowerCase();
  const boundary = (hint.dependencyBoundary ?? "").toLowerCase();
  const text = `${vendor} ${note} ${publicPath} ${boundary}`;
  ...
  if (publicPath.endsWith("assets/browser.ts") && vendor === "npm") return true;
  if (publicPath.includes("processbrowsershim")) return true;
...
export function classifyBoundary(
  ...
  const text =
    `${hint.note ?? ""} ${hint.path ?? ""} ${hint.dependencyBoundary ?? ""}`.toLowerCase();
