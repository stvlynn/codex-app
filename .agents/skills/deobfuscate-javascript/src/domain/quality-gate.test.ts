...
`;
  ...
  test("allows semantic theme data modules with registry-shaped objects", () => {
    ...
    const report = analyzeSource(
      source,
      path.join("src", "themes", "codexDark.ts"),
      {
        ...DEFAULT_OPTIONS,
        maxFlatExports: 1,
      },
  ...
  test("allows large semantic command metadata modules", () => {
    ...
    const report = analyzeSource(
      source,
      path.join("src", "commands", "electronMenuShortcutsImpl.ts"),
      {
        ...DEFAULT_OPTIONS,
      },
    );
  ...
  test("anti-stall: a fully promoted tree passes the drain checks", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        chunks: {
          "button-bq66r8jD": { path: "ui/button.tsx", status: "done" },
        },
      }),
    );
    const reports = analyzeFullRestorationCoverage(targetDir);
  ...
  test("full-restoration coverage rejects app panel empty boundary placeholders", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        boundaries: {
          "pdf-preview-panel-Bo4vARUp": {
            path: "boundaries/pdf-preview-panel.ts",
            boundary: true,
            vendor: "diagram/doc",
          },
        },
  ...
  test("full-restoration coverage rejects app feature typed facades", () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "// TYPED BOUNDARY FACADE (generated by src/infrastructure/make-facade.ts).", 
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        chunks: {
          "review-file-source-tab-C96M0tZ-": {
            path: "review/ReviewFileSourceTab.tsx",
            status: "accepted",
          },
        },
      }),
...
describe("checkFormatting", () => {
  test("flags each file prettier --check reports as unformatted", () => {
    ...
    const reports = checkFormatting("src", fakeRun);
  ...
  test("returns no issues when everything is prettier-clean", () => {
    const reports = checkFormatting("src", () => ({
      ok: true,
