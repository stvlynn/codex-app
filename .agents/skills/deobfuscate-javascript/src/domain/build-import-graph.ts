...
export type ManifestStages = {
   ...
   * semantic path (via `promote-organized.ts`). `src/` is empty until
...
export function parseImportsExports(
  ...
  const recordImport = (
    ...
    imports.push({
      source: sourceStr,
      target,
      kind: cls.kind as ImportEdge["kind"],
...
export function buildImportGraph(
  ...
  while (queue.length > 0) {
    ...
    // or promoted into src/. Consumers import the bare specifier instead.
...
const USAGE =
  "Usage: bun src/domain/build-import-graph.ts [<entry.js>] --target <dir> [--root <dir>] " +
