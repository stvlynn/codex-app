...
import { formatPath } from "../infrastructure/format.ts";
...
const SHORT_ALLOWLIST = new Set([
  ...
  "bg",
  "by",
  "d", // SVG path data attribute (common in icon files)
  "do",
  "el",
...
function collectExportNames(
  ...
  for (const statement of ast.program.body) {
    ...
    if (t.isExportAllDeclaration(statement)) {
      const exported = nodeName(
        (
          statement as unknown as {
            exported?: t.Identifier | t.StringLiteral | null;
          }
        ).exported,
      );
      if (exported) {
...
function isTypeOnlyIdentifierPath(identifierPath: {
  ...
  while (current?.parentPath) {
    const parent: any = current.parentPath;
...
function parameterHasTypeAnnotation(
  ...
  return t.isAssignmentPattern(param) && parameterHasTypeAnnotation(param.left as t.Function["params"][number]);
}
  ...
  traverse(ast, {
    ...
    ExportNamedDeclaration(path) {
      ...
      for (const spec of path.node.specifiers) {
        ...
        if (sourceValue != null && !isHiddenCheckpointImport(sourceValue)) {
          const imported =
            spec.local.type === "Identifier"
              ? spec.local.name
              : (spec.local as unknown as t.StringLiteral).value;
          const exported =
            spec.exported.type === "Identifier"
              ? spec.exported.name
              : (spec.exported as unknown as t.StringLiteral).value;
          if (
            MECHANICAL_IMPORT_BINDING_RE.test(imported) ||
        ...
        else publicNames.add((spec.exported as unknown as t.StringLiteral).value);
        const localName =
          spec.local.type === "Identifier" ? spec.local.name : (spec.local as unknown as t.StringLiteral).value;
        const publicName =
          spec.exported.type === "Identifier"
            ? spec.exported.name
            : (spec.exported as unknown as t.StringLiteral).value;
    ...
    emptyReport(file, [
      {
        code: "unformatted",
        message:
          "File is not prettier-formatted. Run src/infrastructure/format.ts on it (promotion formats deliverables automatically).", 
      },
    ]),
  );
}
...
function analyzeOrganizePromoteState(
  ...
  if (checkpoints.size > 0 && !opts.allowOrganizeIncomplete) {
    ...
    if (undrained.length > 0) {
      issues.push({
        ...
        message:
          "Mechanical checkpoints exist under _full/checkpoints/ but were not promoted into the public tree (src/ left empty). Run src/application/plan-organize.ts then src/application/promote-organized.ts until every chunk is promoted, or pass --allow-organize-incomplete for an intermediate run.",
        detail: {
  ...
  // named after its hash chunk basename (e.g. src/button-bq66r8jD/button.tsx).
...
const USAGE =
  "Usage: bun src/domain/quality-gate.ts <file-or-dir> [--json] [--allow-flat] [--allow-mechanical-names] [--allow-missing-provenance] [--allow-untyped] [--vendored] [--allow-organize-incomplete] [--check-format] " +
