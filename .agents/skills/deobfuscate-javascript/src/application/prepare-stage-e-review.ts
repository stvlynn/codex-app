...
function collectPublicFileEntries(targetDir: string): StageEFileEntry[] {
  return walkPublicSourceFiles(targetDir).map((absolutePath) => {
    ...
    return {
      absolutePath,
      publicRelativePath: toReviewerRelativePath(targetDir, absolutePath),
...
function buildBatches(
  ...
  return batches.map((batch, index) => ({
    ...
    files: batch.map((file) => file.publicRelativePath),
  }));
}
...
export function prepareStageEReviewPackets(
  ...
  const checklistPath = path.resolve(
    options.checklistPath ??
      path.join(import.meta.dir, "..", "infrastructure", "acceptance-checklist.md"),
  );
  const outDir = path.resolve(
    options.outDir ?? path.join(targetDir, ".deobfuscate-javascript", "_stage-e"),
  );
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "",
      "Generated reviewer-agent payloads for the public source tree. These files are hidden under the deobfuscation workspace and are not public deliverables.",
      "",
  ...
  const filesByRelativePath = new Map(
    files.map((file) => [file.publicRelativePath, file]),
  );
...
export function verifyStageEReviewPackets(
  ...
  const publicFiles = publicEntries.map((file) => file.publicRelativePath);
  ...
  if (!Array.isArray(index.files)) {
    ...
    for (const entry of publicEntries) {
      const metadata = indexedMetadataByPath.get(entry.publicRelativePath);
      if (!metadata) continue;
      if (metadata.bytes !== entry.bytes || metadata.sha256 !== entry.sha256) {
        issues.push({
          code: "file-content-mismatch",
          message: `${entry.publicRelativePath} differs from index metadata; regenerate Stage E packets`,
        });
      }
    }
  }
  ...
  for (const batch of index.batches) {
    ...
    const currentBatchBytes = batch.files.reduce((sum, file) => {
      const entry = publicEntries.find(
        (candidate) => candidate.publicRelativePath === file,
      );
...
function usage(): never {
  console.error(
    "Usage: bun src/application/prepare-stage-e-review.ts <target-dir> [--out-dir DIR] [--checklist PATH] [--max-files N] [--max-bytes N] [--verify]",
  );
  process.exit(64);
}
