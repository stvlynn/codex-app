...
import type { Manifest, ManifestFile } from "../domain/build-import-graph.ts";
...
describe("planOrganize (shape heuristics)", () => {
  ...
  test("npm-leaf and faced chunks are excluded from the plan", () => {
    ...
    const p = planOrganize({ manifest: m, target: "src", readCheckpoint: (b) => checkpoints[b] ?? null });
  ...
  test("an existing IMPORT_MAP mapping is reused", () => {
    ...
    const p = planOrganize({
      ...
      importMap: {
        chunks: {
          "copy-to-clipboard-Gh1jKl34": { path: "utils/copy-to-clipboard.ts", status: "done" },
        },
      },
    });
...
describe("applyPlan", () => {
  ...
  test("is idempotent: a second apply skips already-organized chunks", () => {
    ...
    const plan = planOrganize({ manifest, target: "src" });
