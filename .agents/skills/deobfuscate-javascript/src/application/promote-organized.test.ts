...
import type { ManifestFile } from "../domain/build-import-graph.ts";
...
function org(
  domain: string,
  semanticPath: string,
  recipe: NonNullable<ManifestFile["organization"]>["recipe"],
  classification: NonNullable<ManifestFile["organization"]>["classification"],
...
describe("relativeImport / buildImportMappings", () => {
  ...
  test("buildImportMappings only maps producers already promoted", () => {
    ...
    const mappings = buildImportMappings(chunk, "ui/panel.tsx", {
      chunks: {
        ...
        "b-X1Y2Z3b": { path: "utils/b.ts", status: "pending" }, // not done → skipped
      },
    });
  ...
  test("buildImportMappings rewrites vendor-data imports to the bare specifier", () => {
    ...
    } as unknown as import("../domain/build-import-graph.ts").Manifest;
...
describe("promoteOrganized", () => {
  ...
  test("promotes recipes + manual candidate, updates IMPORT_MAP, rewrites imports", () => {
    ...
    expect(importMap.chunks["format-thing-AbCdEf12"].path).toBe(
  ...
  test("promotes a split directory candidate and records its index entry", () => {
    ...
    expect(importMap.chunks["score-query-match-AbCdEf12"].path).toBe(
