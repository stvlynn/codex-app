...
import type { SymbolEntry } from "../infrastructure/extract.ts";
...
export function formatChecklist(plan: Plan, outDir: string): string {
  ...
  lines.push(
    "3. Apply once: `bun src/infrastructure/apply.ts <input> <outDir>/renames.json --out <output>`.",
  );
  lines.push(
...
const USAGE =
  "Usage: bun src/application/plan.ts <symbols.json|-> --out-dir <dir> " +
