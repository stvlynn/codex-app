...
import {
  ...
  type Manifest,
  type ManifestFile,
} from "../domain/build-import-graph.ts";
import {
  acquireLock,
  computeFrontier,
  LockHeldError,
...
export function buildImportMappings(
  ...
  for (const imp of chunk.imports ?? []) {
    // Vendor data / npm-leaf chunks: redirect to the bare specifier regardless
    // of the (stale) edge kind recorded before the target's content was
    // fingerprinted. Regular npm-leaf chunks record the package in
    // `npmPackage`; vendor-data chunks also set `vendorSpecifier`.
    const target = manifest?.files?.[imp.target];
    if (target?.kind === "npm-leaf") {
      const bareSpecifier = target.vendorSpecifier ?? target.npmPackage;
      if (bareSpecifier) {
        mappings.push({
          source: imp.source,
          to: bareSpecifier,
    ...
    if (!producer?.path || producer.status !== "done") continue;
    mappings.push({
      source: imp.source,
      to: relativeImport(semanticPath, producer.path),
...
export function promoteOrganized(
  ...
  const writeBuilt = (built: Built): void => {
    for (const f of built.files) {
      ...
      // so promoted files in src/ are never raw @babel/generator output
...
const USAGE = `Usage: bun src/application/promote-organized.ts --target <dir> [options]
...
Options:
  --target <dir>     Target restore root (e.g. src) to write promoted files into (required).
