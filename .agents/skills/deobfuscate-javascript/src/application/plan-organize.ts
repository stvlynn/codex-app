...
import { pathsForTarget, setOrganization } from "../domain/ledger.ts";
import {
  classifyBoundary,
  isKnownTerminalBoundaryChunk,
} from "../domain/chunk-classification.ts";
...
function classify(
  ...
  const mapped = ctx.importChunk?.path;
  if (mapped) {
    return mk({
...
const USAGE = `Usage: bun src/application/plan-organize.ts --target <dir> [options]
...
src/domain/ledger.ts set-organization), then re-run with --apply to write approved entries
into the manifest.

Options:
  --target <dir>         Target restore root (e.g. src) whose IMPORT_MAP.json to read (required).
