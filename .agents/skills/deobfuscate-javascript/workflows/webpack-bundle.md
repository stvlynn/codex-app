...
- No usable sourcemap (run `src/application/sourcemap-check.ts` first — if `.map` exists, recover originals instead).
...
bun src/infrastructure/polish.ts "$WS/webcracked/modules/3658.js" --rename --fast \
